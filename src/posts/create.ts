...
module.exports = function (Posts: {
  ...
  uploads: { sync: (pid: string) => unknown };
}) {
  ...
  Posts.create = async (data: CreateData) => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
    const pid: string = (await db.incrObjectField(
      "global",
      "nextPid",
    )) as string;
    ...
    let result: { post: PostData } = (await plugins.hooks.fire(
      "filter:post.create",
    ...
    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
    const topicData: { cid?: string; pinned: boolean } =
      (await topics.getTopicFields(tid, ["cid", "pinned"])) as {
        cid?: string;
        pinned: boolean;
      };
